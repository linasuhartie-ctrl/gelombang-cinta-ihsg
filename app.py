...
def compute_matrix_waves(df):
    ...
    df["vol_wave"] = (
        (mf_mult * df["Volume"]).rolling(20).mean()
        / df["Volume"].rolling(20).mean().replace(0, 0.001) * 100
...
def main():
    ...
    with tab_scan:
            ...
            def process(ticker):
                ...
                if mode == "Wave Matrix":
                    if strategy == "Level Garis Putih" and struct_range:
                        matched = struct_range[0] <= latest["struct_wave"] <= struct_range[1]
                ...
                return {
                    "Asset": ticker.replace(".JK", "").replace("-USD", ""),
        ...
        if st.session_state.results:
            ...
            st.info("Belum ada hasil scan. Klik RUN SCAN dulu.")

    with tab_ai:
        if st.session_state.results:
            selected = st.selectbox("Pilih Aset", [r["Asset"] for r in st.session_state.results])
            full_ticker = selected + suffix
            df_p = fetch_data(full_ticker, timeframe)
            df_p = compute_matrix_waves(df_p)
            ...
            if df_p is not None and len(df_p) > 30:
                ...
                if st.button("🪄 Start Deep Journey Insight", use_container_width=True):
                    if client is None:
                        st.error("AI insight belum aktif karena GROQ_KEY belum tersedia.")
                    else:
                        with st.spinner("Menganalisis 20 candle terakhir..."):
                            st.markdown(get_ai_insight(client, selected, df_p))
        else:
            st.info("Hasil scan belum tersedia.")
