...
# ──────────────────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Aulsome Matrix Pro V6.3",
    page_icon="🔮",
    layout="wide",
...
# ──────────────────────────────────────────────────────────────────────────────
# 3. SENIOR AI PROMPT BUILDER
# ──────────────────────────────────────────────────────────────────────────────
def build_ai_prompt(asset, df, timeframe):
...
Tugas Anda adalah memberikan analisis objektif untuk aset {asset} ({timeframe}) berdasarkan data teknikal berikut.

DEFINISI INDIKATOR MATRIX (PENTING):
- struct_wave (Garis Putih): Mengukur Market Structure. Jika <= -80 artinya jenuh jual (Bottoming), >= 80 artinya jenuh beli (Topping).
- vol_wave (Garis Kuning): Mengukur Volume Flow / Akumulasi Bandar. > 0 artinya ada akumulasi, < 0 artinya distribusi.
- dom_wave (Garis Ungu): Momentum dominasi Buyer vs Seller.
- inflow_ratio: > 1.0 berarti ada uang besar masuk melebihi rata-rata.
...
2. MOMENTUM & FLOW: Analisis interaksi antara vol_wave (Garis Kuning) dan inflow_ratio. Jika vol_wave positif namun harga stagnan, apa artinya?
3. SENSITIVITY: Apakah rsi menunjukkan divergensi atau konfirmasi tren? Bagaimana posisi struct_wave (Garis Putih)?
...
def main():
    init_state()
    st.title("🔮 Aulsome Matrix Pro V6.3")
    ...
    with tab_deep:
                        ...
                        with st.spinner("Analyzing Market Structure & Order Flow..."):
                            ...
                            try:
                                resp = client.chat.completions.create(
                                    ...
                                    temperature=0.3 # Membuat jawaban lebih analitik dan tidak halu
                                )
        ...
        else: st.info("Lakukan scan market terlebih dahulu.")
