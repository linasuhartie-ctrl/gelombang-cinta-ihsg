...
# 0. API & CONFIG
# ──────────────────────────────────────────────────────────────────────────────
# Pastikan GEMINI_KEY sudah diisi di dashboard Streamlit Secrets
try:
...
def get_ai_insight(asset, pattern, white, vol, quality, price):
    try:
        # MENGGUNAKAN gemini-1.5-flash UNTUK MENGHINDARI 404
    ...
    if st.sidebar.button(f"RUN SCAN ({len(tickers)} ASSETS)", use_container_width=True):
        ...
        def process_ticker(t):
            ...
            if df is not None and len(df) >= 10:
                ...
                if mode == "Wave Matrix":
                    if strategy == "Level Garis Putih":
                        # Gunakan struct_range dari slider
