...
# --- CONFIG ---
st.set_page_config(
    page_title="Aulsome Matrix Pro V4.8",
    page_icon="🔮",
    layout="wide",
...
# --- AI PROMPT BUILDER (NEW ORDER FLOW VERSION) ---
def build_ai_prompt(asset, df):
    # Mengambil data yang lebih relevan untuk analisis tren (30 hari terakhir)
    lookback = df.tail(30).copy()
    # Memilih kolom yang relevan saja untuk dikirim agar token efisien
...
3. SENSITIVITY: Apakah RSI dan Stochastic menunjukkan divergensi atau konfirmasi tren?
...
def check_sniper(df):
    ...
    vol_spike = l["Volume"] > (vol_sma * 1.2)
...
def detect_patterns(df):
    ...
    if (min(c["Open"], c["Close"]) - c["Low"]) > 2*body: return "Hammer"
...
def main():
    init_state()
    st.title("🔮 Aulsome Matrix Pro V4.8")
    ...
    with st.sidebar:
        ...
        if mode == "Inflow Detector 💰":
            ...
            strategy = st.selectbox("Pola", ["Hammer", "Bullish Engulfing"])
